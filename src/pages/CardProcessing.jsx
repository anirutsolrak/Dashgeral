...
function CardProcessing({ filters }) {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            let responseData = {}; // Initialize responseData as an empty object

            if (filters.agreement && filters.agreement !== 'all') {
                // Lógica CORRIGIDA: Se filtro de CONVÊNIO ATIVO, use as funções FILTRADAS e MAPEIE os resultados PARA A ESTRUTURA ESPERADA PELOS KPICARDS
                const convenioFilter = filters.agreement;

                // Busque CADA KPI FILTRADO individualmente (retorna um ARRAY de objetos - um para cada convenio)
                const integrationRateDataArray = await getTaxaIntegracaoFiltered(convenioFilter); // Agora retorna ARRAY
                const accountsCreatedDataArray = await getContasCriadasFiltered(convenioFilter); // Agora retorna ARRAY
                const cardsSentDataArray = await getCartoesEnviadosFiltered(convenioFilter); // Agora retorna ARRAY
                const insuranceProposalsDataArray = await getPropostasSeguroFiltered(convenioFilter); // Agora retorna ARRAY
                const valorDataArray = await getPropostasSeguroValorFiltered(convenioFilter); // Agora retorna ARRAY
                const cessaoDataArray = await getCasosCessaoFiltered(convenioFilter); // Agora retorna ARRAY


                // MAPEAMENTO FINAL - Ajustar a estrutura do objeto responseData para corresponder EXATAMENTE ao que KPICards esperam
                responseData = {
                    integrationRate: integrationRateDataArray?.[0] || {}, // **PEGAR O PRIMEIRO ELEMENTO DO ARRAY (ASSUMINDO QUE SEMPRE HAVERÁ 1)**
                    accountsCreated: accountsCreatedDataArray?.[0] || {},   // **PEGAR O PRIMEIRO ELEMENTO DO ARRAY**
                    cardsSent: cardsSentDataArray?.[0] || {},       // **PEGAR O PRIMEIRO ELEMENTO DO ARRAY**
                    insuranceProposals: insuranceProposalsDataArray?.[0] || {}, // **PEGAR O PRIMEIRO ELEMENTO DO ARRAY**
                    valorData: valorDataArray?.[0] || {},           // **PEGAR O PRIMEIRO ELEMENTO DO ARRAY**
                    cessaoData: cessaoDataArray?.[0] || {},          // **PEGAR O PRIMEIRO ELEMENTO DO ARRAY**
                    digitizedStopReasons: data?.digitizedStopReasons, // **MANTÉM OS MOTIVOS NÃO FILTRADOS - OU AJUSTE SE QUISER FILTRAR MOTIVOS TAMBÉM**
                    nonDigitizedReasonsBreakdown: data?.nonDigitizedReasonsBreakdown, // **MANTÉM OS MOTIVOS NÃO FILTRADOS - OU AJUSTE SE QUISER FILTRAR MOTIVOS TAMBÉM**
                    accountsMotivesData : data?.accountsMotivesData, // **MANTÉM OS MOTIVOS NÃO FILTRADOS - OU AJUSTE SE QUISER FILTRAR MOTIVOS TAMBÉM**
                    contasCriadasMotivos: data?.contasCriadasMotivos, // **MANTÉM OS MOTIVOS NÃO FILTRADOS - OU AJUSTE SE QUISER FILTRAR MOTIVOS TAMBÉM**
                    contasNaoCriadasMotivos: data?.contasNaoCriadasMotivos, // **MANTÉM OS MOTIVOS NÃO FILTRADOS - OU AJUSTE SE QUISER FILTRAR MOTIVOS TAMBÉM**
                    filters: data?.filters // Mantenha os filtros originais
                };


            } else {
                // Se NÃO houver filtro de convênio selecionado (ou for 'all'), use a função original para dados NÃO filtrados
                responseData = await getCardProcessingData(); // Chama getCardProcessingData APENAS para dados "Totais"
            }

            console.log("Response Data from fetchData:", responseData); // Log response data (útil para debug)
            setData(responseData);
            setError(null);
        } catch (err) {
            console.error('Error fetching data:', error);
            setError('Failed to load dashboard data');
            setData(getMockData('cardProcessing')); // Fallback para mock data em caso de erro
        } finally {
            setLoading(false);
        }
    };
    ...
    const getModalContent = (kpiType) => {
        console.log(`Data for ${kpiType} modal:`, data);
        ...
        switch (kpiType) {
                ...
                return (
                    ...
                    <div className="space-y-6" style={{ maxWidth: '50vw' }}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {renderPieChartContent(digitizedStopReasons, 'Motivos de Parada - Digitadas')}
                            {renderPieChartContent(nonDigitizedReasonsBreakdown, 'Motivos de Não Digitação')}
                        </div>
                    </div>
                );
            }
            ...
            case 'accounts': {
                ...
                const created = accountsCreatedData?.contas_criadas_sucesso || 0;
                const notCreated = accountsCreatedData?.contas_nao_criadas || 0;
                ...
                return (
                    <div className="space-y-6" style={{ maxWidth: '50vw' }}>
                        <div className="bg-white rounded-lg shadow-md" style={{ maxWidth: '35vw', margin: '0 auto' }}>
                            {console.log("Dados para ChartComponent (bar) - Accounts:", created, notCreated)}
                            <ChartComponent
                                type="bar"
                                data={{
                                    labels: [''],
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {renderPieChartContent(createdReasons, 'Motivos - Contas Criadas')}
                            {renderPieChartContent(notCreatedReasons, 'Motivos - Contas Não Criadas')}
                        </div>
                    </div>
                );
            }
            ...
            default:
                console.log("getModalContent case: default - kpiType:", kpiType);
                return (
            ...
            <div className="bg-white rounded-lg p-3 shadow-md">
                ...
                <div className="custom-pie-legend grid grid-cols-2 gap-x-4 max-w-md mx-auto">
                    {chartData.map((item, index) => (
                        <div key={index} className="flex items-center mb-2" style={{ width: '100%', minWidth: '150px' }}>
                            ...
                            <div className="text-xs" style={{ overflowWrap: 'break-word', wordBreak: 'break-word' }}>
                                {item.reason}:
    ...
    const getWorkflowContent = (kpiType, workflowType) => {
        const workflowContentMap = {
            integration: {
                flowchart: (
                    <div>
                        ...
                        <p className="text-xs text-black-500 mt-2 text-center">
                            <span className="doc-link" onClick={openFullDocumentModal}>
                                Consulte o documento completo para detalhes.
                            </span>
                            <span className="doc-link text-blue-500 cursor-pointer" onClick={openFullDocumentModal}>
                                AQUI
                            </span>
                        </p>
                    </div>
                ...
                orgchart: (
                    <div>
                        <div className="organograma-react-orgchart-container">
                            <OrgChart
                                tree={integrationOrganogramData}
            ...
            cards: {
                ...
                docs: (
                    <div>
                        <h4 className="font-semibold mb-2">POP - Digit ação de Propostas (Placeholder)</h4>
                        <p>Leitura de PDF do POP será implementada aqui:</p>
                        <div className="pdf-placeholder-container mt-2 p-4 border rounded-md bg-gray-100">
                            <p className="text-sm text-gray-700 italic">
                                [Placeholder - Leitor de PDF será implementado aqui para exibir o documento "POP - Digitação de Propostas no Função - Passo a Passo.pdf"]
            ...
            insuranceProposals: {
                ...
                orgchart: (
                    <div>
                        <div className="organograma-react-orgchart-container">
                            <OrgChart
                                tree={insuranceProposalsOrganogramData}
                                NodeComponent={MyNodeComponent}
                            />
                        </div>
                    </div>
    ...
    console.log("Data object in CardProcessing:", data);
    ...
    return (
        <div data-name="card-processing" className="dashboard-container">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <KPICard
                    key="taxa-integracao-card"
                    title="Taxa de Integração"
                    value={formatPercentage(data?.integrationRate?.taxa_percentual) || 'N/A'}
                    subtitle='Das contas para digitar'
                    icon="fa-sync"
                    color="purple"
                    onClick={() => handleKPIClick('integration')}
                    onWorkflowClick={(type) => handleWorkflowClick('integration', type)}
                    summaryContent={( // **summaryContent - APENAS o conteúdo de resumo para Taxa de Integração**
                        <div className="space-y-6"> {/* Container para o resumo - opcional, mas pode ajudar no layout */}


                            <div className="grid grid-cols-2 ">
                                <div className=" bg-blue-50 rounded-lg text-center"> {/* Text-center para centralizar texto */}
                                    <p className="text-sm text-blue-600">Propostas Digitadas</p>
                                    <p className="text-lg font-bold">{data?.integrationRate?.propostas_digitadas_sucesso || 0}</p>
                                </div>
                                <div className=" bg-green-50 rounded-lg text-center"> {/* Text-center para centralizar texto */}
                                    <p className="text-sm text-red-600">Propostas Não digitadas</p>
                                    <p className="text-lg font-bold">{data?.integrationRate?.propostas_nao_digitadas || 0}</p>
                                </div>
                    ...
                    summaryContent={( // **summaryContent - APENAS o conteúdo de resumo para Contas Criadas**
                        ...
                        <div className="grid grid-cols-2 ">
                            <div className=" bg-green-50 rounded-lg text-center"> {/* Text-center para centralizar texto */}
                                ...
                                <p className="text-lg font-bold">{data?.accountsCreated?.contas_criadas_sucesso || 0}</p>
                            </div>
                            <div className=" bg-red-50 rounded-lg text-center"> {/* Text-center para centralizar texto */}
                                <p className="text-sm text-red-600">Contas Não Criadas</p>
                                <p className="text-lg font-bold">{data?.accountsCreated?.contas_nao_criadas || 0}</p>
                            </div>
                ...
                />
                <KPICard
                    key="cartoes-enviados-card"
                    title="Cartões Enviados"
                    value={data?.cardsSent?.cartoes_enviados || 'N/A'}
                    subtitle='Cartões que sairam pra entrega'
                    icon="fa-credit-card"
                    color="orange"
                    onClick={() => handleKPIClick('cards')}
                    onWorkflowClick={(type) => handleWorkflowClick('cards', type)}
                    summaryContent={( // **summaryContent - APENAS o conteúdo de resumo para Cartões Enviados**
                        ...
                        <div className="grid grid-cols-2 ">
                            <div className=" bg-blue-50 rounded-lg text-center"> {/* Text-center para centralizar texto */}
                                ...
                                <p className="text-lg font-bold">{data?.cardsSent?.total_contas_criadas || 0}</p>
                            </div>
                            <div className=" bg-green-50 rounded-lg text-center"> {/* Text-center para centralizar texto */}
                                <p className="text-sm text-green-600">Cartões Enviados</p>
                                <p className="text-lg font-bold">{data?.cardsSent?.cartoes_enviados || 0}</p>
                            </div>
                ...
                <KPICard
                    ...
                    summaryContent={( // **summaryContent - APENAS o conteúdo de resumo para Propostas com Seguro**
                        ...
                        <div className="grid grid-cols-2 ">
                            <div className=" bg-blue-50 rounded-lg text-center"> {/* Text-center para centralizar texto */}
                                <p className="text-sm text-blue-600">Total de Propostas</p>
                                <p className="text-lg font-bold">{data?.insuranceProposals?.total_propostas || 0}</p>
                            </div>
                            <div className=" bg-green-50 rounded-lg text-center"> {/* Text-center para centralizar texto */}
                                <p className="text-sm text-green-600">Propostas com Seguro</p>
                                <p className="text-lg font-bold">{data?.insuranceProposals?.propostas_com_seguro || 0}</p>
                            </div>
